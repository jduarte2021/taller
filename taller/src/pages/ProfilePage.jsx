      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="rounded-2xl p-6 mb-6 flex items-center gap-6" style={{ background: t.bgCard, border: `1px solid ${t.border}` }}>
          <div className="relative flex-shrink-0">
            {avatarSrc ? (
              <img src={avatarSrc} alt="avatar" className="w-24 h-24 rounded-full object-cover" style={{ border: `3px solid ${t.accent}` }} />
            ) : (
              <div className="w-24 h-24 rounded-full flex items-center justify-center text-3xl font-black" style={{ background: `${t.accent}20`, color: t.accent, border: `3px solid ${t.accent}` }}>{initials}</div>
            )}
          ...
          <div className="flex-1">
            ...
            {profileImage && <button type="button" onClick={() => setProfileImage(null)} className="ml-2 text-xs" style={{ color: "#f87171" }}>✕ Quitar</button>}
          </div>
        </div>
        <form onSubmit={handleSubmit} className="rounded-2xl p-6 space-y-4" style={{ background: t.bgCard, border: `1px solid ${t.border}` }}>
          ...
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider mb-1.5" style={{ color: t.textMuted }}>Cargo {isSuperAdmin && <span style={{ color: t.accent }}>(editable)</span>}</label>
            {isSuperAdmin ? (
